...
define(['jquery'], function($) {
    ...
    var Tree = function(selector, selectCallback) {
        ...
        // Create a namespace to ensure we don't bind handlers multiple times.
        this.namespace = '.tree';
    ...
    Tree.prototype.expandGroup = function(item) {
        ...
        // Ignore nodes that are explicitly marked as not expandable or are already expanded.
    ...
    Tree.prototype.bindEventHandlers = function() {
        // Remove handlers already bound with this namespace
        this.treeRoot.off(this.namespace);

        // Bind event handlers to the tree items. Use event delegates to allow
        // for dynamically loaded parts of the tree.
        this.treeRoot.on('click' + this.namespace, SELECTORS.ITEM, this.handleClick.bind(this));
        this.treeRoot.on('keydown' + this.namespace, SELECTORS.ITEM, this.handleKeyDown.bind(this));
        this.treeRoot.on('focus' + this.namespace, SELECTORS.ITEM, this.handleFocus.bind(this));
    };
